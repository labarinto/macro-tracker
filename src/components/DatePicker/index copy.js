...
const DatePicker = (props) => {
    ...
    useEffect( () => {
        const getFirstDateForMonth = (currentDate) => { // get which day, 1 starts for current month
            let firstDate = new Date(`${currentDate.getMonth() + 1}/1/${currentDate.getFullYear()}`);
    ...
    const changeDate = (nextDate) => {
        const newDate = `${selectedMonth + 1}-${nextDate}-${selectedYear}`;
        setSelectedDate(new Date(newDate));
        props.onChange(newDate);
    }

    const nextPrevDate = (nextDate, month, year) => {
        const newDate = `${month}-${nextDate}-${year}`;
        setSelectedDate(new Date(newDate));
    ...
    const updateUICalendar = (date, month = selectedMonth, year = selectedYear) => {
        ...
        setDate(new Date(`${month + 1}-${date}-${year}`));
    }
        ...
        <div className={classes.DatePicker}>
            <div className={classes.Title}>
                ...
                <h3 onClick={props.onToggle}>{
                    `${MONTH[selectedDate.getMonth()].slice(0,3)} ${selectedDate.getDate()} ${selectedDate.getFullYear()}`
                }</h3>
